<script lang="ts">
    ...
    let configs=$state({
      ...
      childrens:undefined as undefined|Map<'center'|'left'|"right",{
        ref?:HTMLElement
      }>
    })
...
{:else}
<MessageComponent nameComponent={NAME_COMP[client.browser?.language??'en']??''} color="danger" description={MESSAGE_ERROR.main[client.browser?.language??'en']??''}/>
{/if}
<style lang="scss">
    .nav-main-root{
